...
constexpr const char* kMenuItemBundleTitle = "2-Up Artifact Bundle";
...
bool RegisterMenus() {
    ...
    gBundleExecuteProc = ASCallbackCreateProto(AVExecuteProc, ExecuteTwoUpArtifactBundleExport);
    gPluginBundleMenuItem = AVMenuItemNew(
        kMenuItemBundleTitle,
        "AIMP:TwoUpArtifactBundle",
        nullptr,
        true,
        NO_SHORTCUT,
        0,
    ...
    if (gPluginBundleMenuItem == nullptr) {
        return false;
    }
    AVMenuAddMenuItem(gPluginSubMenu, gPluginBundleMenuItem, APPEND_MENUITEM);
    return true;
}
...
ACCB1 void ACCB2 ExecuteTwoUpArtifactBundleExport(void* clientData) {
    DURING
        ...
        const auto tempDir = std::filesystem::temp_directory_path(fsError);
        if (fsError) {
            ShowInfoDialog("Kan temp map niet bepalen voor artifact bundle.");
            E_RETURN_VOID;
        }

        aimp::ArtifactBundleOptions options {};
        options.outputDirectory = tempDir.string();
        options.baseName = "acrobat-imposition-two-up";
        aimp::ArtifactBundlePaths paths {};
        std::string error;
        if (!aimp::WritePlanArtifactBundle(plan, options, paths, error)) {
            ShowInfoDialog("Kon artifact bundle niet schrijven: " + error);
            E_RETURN_VOID;
        }

        std::string message = "2-Up artifact bundle opgeslagen:\n";
        message += paths.jsonPath;
        message += "\n";
        message += paths.auditXmlPath;
        if (!paths.pdfPath.empty()) {
            message += "\n";
            message += paths.pdfPath;
        }
        ShowInfoDialog(message);
    HANDLER
        ShowInfoDialog("Er trad een fout op tijdens artifact bundle export.");
    END_HANDLER
}
...
extern "C" ACCB1 ASBool ACCB2 PluginUnload(void) {
    if (gPluginBundleMenuItem != nullptr) {
        AVMenuItemRemove(gPluginBundleMenuItem);
        gPluginBundleMenuItem = nullptr;
    }
    if (gPluginReportMenuItem != nullptr) {
