...
void PrintUsage() {
    std::cout
        ...
        << "Common options for all modes: [--load-preset <file>] [--save-preset <file>] [--reverse 0|1] [--filter all|even|odd] [--pad-multiple N] [--signature-size N] [--fit-to-slot 0|1] [--rotate-to-fit 0|1] [--source-page-width <pt>] [--source-page-height <pt>] [--audit-out <file.xml>] [--pdf-out <file.pdf>] [--bundle-dir <dir>] [--bundle-base <name>] [--bundle-include-pdf 0|1] [--pdf-header <text>] [--pdf-footer <text>] [--pdf-sheet-number 0|1] [--pdf-bates-enable 0|1] [--pdf-bates-prefix <text>] [--pdf-bates-start N] [--inspect-source-page <N>] [--inspect-sheet <N> --inspect-slot <N>]\n";
}
...
int main(int argc, char** argv) {
    ...
    std::string bundleDirPath;
    std::string bundleBaseName = "imposition-plan";
    bool bundleIncludePdf = true;
    ...
    for (int i = 2; i < argc; ++i) {
        ...
        if (key == "--pages") {
            ...
            bundleDirPath = value;
        } else if (key == "--bundle-base") {
            bundleBaseName = value;
        } else if (key == "--bundle-include-pdf") {
            std::uint32_t raw = 1;
            if (!ParseUInt(value, raw) || raw > 1) {
                std::cerr << "Invalid value for --bundle-include-pdf (expected 0 or 1)\n";
                return 1;
            }
            bundleIncludePdf = (raw == 1);
    ...
    if (!bundleDirPath.empty()) {
        aimp::ArtifactBundleOptions bundleOptions {};
        bundleOptions.outputDirectory = bundleDirPath;
        bundleOptions.baseName = bundleBaseName;
        bundleOptions.includePdf = bundleIncludePdf;
        bundleOptions.pdfOptions = pdfOptions;
        aimp::ArtifactBundlePaths bundlePaths {};
        std::string errorMessage;
        if (!aimp::WritePlanArtifactBundle(plan, bundleOptions, bundlePaths, errorMessage)) {
            std::cerr << "Could not write artifact bundle: " << errorMessage << '\n';
            return 1;
        }
        std::cout << "\n# Bundle output\n";
        std::cout << "json=" << bundlePaths.jsonPath << '\n';
        std::cout << "auditXml=" << bundlePaths.auditXmlPath << '\n';
        if (!bundlePaths.pdfPath.empty()) {
            std::cout << "pdf=" << bundlePaths.pdfPath << '\n';
        }
    }
