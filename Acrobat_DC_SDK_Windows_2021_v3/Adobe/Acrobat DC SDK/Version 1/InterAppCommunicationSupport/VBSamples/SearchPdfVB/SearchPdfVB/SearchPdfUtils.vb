...
#If Win64 Then
	...
	Declare Sub SetQueryLanguageType Lib "DdeProxy.dll" Alias "SetQueryLanguageType" (ByVal language As Short)
	' Set the MaxDocs
	Declare Sub SetQueryMaxDocs Lib "DdeProxy.dll" Alias "SetQueryMaxDocs" (ByVal maxDocs As Short)
	' Override the Query word options
	Declare Sub SetQueryOverideWordOptions Lib "DdeProxy.dll" Alias "SetQueryOverrideWordOptions" ()
	' Set the Query word options
	Declare Sub SetQueryWordOptions Lib "DdeProxy.dll" Alias "SetQueryWordOptions" (ByVal flags As Integer)
	' Set the Query target word
	Declare Sub SetQueryWordTarget Lib "DdeProxy.dll" Alias "SetQueryWordTarget" (ByVal target As String)
	' Set the Sort option with boolean for ascending (0=descend, 1=ascend)
	Declare Sub SetQuerySortOption Lib "DdeProxy.dll" Alias "SetQuerySortOption" (ByVal field As String, ByVal ascending As Short)

	' Set the Index Action Type
	Declare Sub SetIndexAction Lib "DdeProxy.dll" Alias "SetIndexAction" (ByVal action As Short)
	' Set the Index name
	Declare Sub SetIndexName Lib "DdeProxy.dll" Alias "SetIndexName" (ByVal indexName As String)
	' Set the Index temporary name
	Declare Sub SetIndexTempName Lib "DdeProxy.dll" Alias "SetIndexTempName" (ByVal TempIndexName As String)

	' Initialize the Query DDE Conversation
	Declare Sub SrchDDEInitQuery Lib "DdeProxy.dll" Alias "SrchDDEInitQuery" ()
	' Initialize the Index DDE Conversation
	Declare Sub SrchDDEInitIndex Lib "DdeProxy.dll" Alias "SrchDDEInitIndex" ()
	' Send the QueryData packet and terminate DDE Conversation
	Declare Sub SrchDDESendQuery Lib "DdeProxy.dll" Alias "SrchDDESendQuery" ()
	' Send the IndexData packet and terminate DDE Conversation
	Declare Sub SrchDDESendIndex Lib "DdeProxy.dll" Alias "SrchDDESendIndex" ()
#Else
	' Set the Query language
