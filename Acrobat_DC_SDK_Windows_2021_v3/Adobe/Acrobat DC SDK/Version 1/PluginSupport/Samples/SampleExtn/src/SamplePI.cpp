...
/*	Display a message box asking user to run Acrobat in Protected Mode
	or check if the SamplePIBroker.exe is installed properly.
*/
void DisplayMessageWhenPluginNotWorking()
{
	char str[512];
	int j = 0;
	AVAppBeginModal(NULL);
	j = sprintf_s(str, 512, "Please run Acrobat in Protected Mode to use the plugin.\n\n");
	j += sprintf_s(str + j, 512 - j, "Go to Preferences -> Security (Enhanced) -> Enable Protected Mode at startup.\n\n");
	j += sprintf_s(str + j, 512 - j, "If Acrobat is running in Protected Mode, please check if the SamplePIBroker.exe is installed properly at the location,\n<Acrobat_Installed_Path>\\plug_ins\\pi_brokers\\SamplePIBroker.exe.\n\n");
...
ACCB1 void ACCB2 DisplayMessageInBrokerCommand(void *clientData)
{
	...
	{
		//Display a message box asking user to run Acrobat in Protected Mode or check if the SamplePIBroker.exe is installed properly.
		DisplayMessageWhenPluginNotWorking();
	}
