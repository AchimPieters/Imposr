...
void DSEngine::sigGetSigValue( PSSigGetSigValueParams params )
{
	...
	if (params->sigMethod != kPSSigMethodPKCS1)
		return;
	if (params->digestMethod != kDSDigestSHA256)
		return;
	...
	if (ASBoolToBool(params->bGetSigValue) == true) {
		...
		memcpy( params->outSigValueData + 3, params->digestValue, 32 ); // SHA256 is 32 bytes
	}
...
void DSEngine::sigValidate( PSSigValidateParams params )
{
	...
	// We CHEAT here (no crypto library!) - YOU must not!
	// Confirm our signature is the same as the digest (all SHA256 are 32 bytes)
	if (memcmp( params->sigValueData +3, params->digestValue, 32 ) != 0)
		return;
