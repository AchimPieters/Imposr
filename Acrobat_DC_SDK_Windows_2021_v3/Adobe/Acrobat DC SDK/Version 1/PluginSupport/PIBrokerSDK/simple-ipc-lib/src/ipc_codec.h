...
using ValueTypeUnsigned = uint64_t;
#else
using ValueType		    = int;
using ValueTypeUnsigned = unsigned int;
#endif
...
public:
  ...
  bool OnWord(void* bits, int tag) {
    SetHeaderNext(tag);
#if  ACROBAT_SANDBOX 
    data_.push_back(reinterpret_cast<int64_t>(bits));
#else
  ...
  void SetMsgId(int id) {
    ...
    data_[1] =((int64_t)id);
#else
    data_[1] = reinterpret_cast<void*>((intptr_t)id);
#endif
  }
...
private:
...
#if  ACROBAT_SANDBOX
    data_[++index_] = ((int64_t)v);
#else
    data_[++index_] = reinterpret_cast<void*>((intptr_t)v);
#endif
  }
  ...
  void PushBack(ValueTypeUnsigned v) {
#if  ACROBAT_SANDBOX
      data_.push_back(v);
#else
...
#if  ACROBAT_SANDBOX
    const int times = sizeof(ValueTypeUnsigned) / sizeof(s[0]);
#else
    const int times = sizeof(IPCVoidPtrVector::value_type) / sizeof(s[0]);
#endif
    size_t it = 0;
    do {
		ValueTypeUnsigned v = 0;
  ...
  ValueTypeUnsigned PackChar(char c, int offset) const {
    return  static_cast<ValueTypeUnsigned>(static_cast<unsigned char>(c)) << (offset * 8);
  }
...
public:
...
#if  ACROBAT_SANDBOX
    if (data_.size() % sizeof(int64_t) == 0) {
#else
  ...
  void Reset() {
    state_ = DEC_S_START;
    e_count_ = -1;
    d_count_ = static_cast<ValueType>(-1);
    next_char_ = 0;
    res_ = DEC_NONE;
  }
...
private:
  ...
  Result StateStart() {
    ...
    if (!HasEnoughUnProcessed(4))
      return DEC_MOREDATA;
	ValueType i0 = ReadNextInt();
    if (Encoder::ENC_HEADER != i0)
      return DEC_ERROR;
	ValueType msg_id = ReadNextInt();
    if (msg_id < 0)
      return DEC_ERROR;
  ...
  Result StateHeader() {
    ...
    if (d_count_ < (ValueType)items_.size())
      return DEC_ERROR;
  ...
  ValueType ReadNextInt() {
    ValueType v  = *reinterpret_cast<ValueType*>(&data_[next_char_]);
#if  ACROBAT_SANDBOX
    next_char_ += sizeof(int64_t);
#else
...
#if  ACROBAT_SANDBOX
    next_char_ += sz_rounded * sizeof(int64_t);
#else
...
#if  ACROBAT_SANDBOX
    return ((data_.size() - next_char_) >= (ints * sizeof(int64_t)));
#else
...
#if  ACROBAT_SANDBOX
    return (sz + (sizeof(int64_t)-1))/sizeof(int64_t);
#else
