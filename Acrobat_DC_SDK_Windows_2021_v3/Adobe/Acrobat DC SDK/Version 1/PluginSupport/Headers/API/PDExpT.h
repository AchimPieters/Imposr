...
/**
(Optional) A callback for PDCryptFilterHandler. Callbacks that conform to this prototype are called to
check if the crypt filter's encryption is changed.

@param dP The document whose security handler contains this crypt filter.
*/
typedef ACCBPROTO1 ASBool(ACCBPROTO2 *PDCryptFilterIsEncryptionChangedProc)(CosDoc dP);
...
typedef struct _t_PDCryptFilterHandler {
	...
	PDCryptFilterIsEncryptionChangedProc IsEncryptionChanged;
