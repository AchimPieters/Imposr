...
/**
	...
	<p><code>theThing = (HWND) AVWindowGetPlatformThingEx( theAVWindow); </code></p>
...
/**
	...
	@return The newly created window. 
	@exception genErrNoMemory AVWindowNew AVWindowGetPlatformThingEx 
 ...
 */
 ...
 /**
 AVStorageRegisterNewStorage registers a new Desktop storage
 @param fs_name - ASAtom for the FileSys name which is to be used in File Operations for this storage file/folders
 Thus fs_name ASFileSys should be registered before calling this HFT
 @param info - AVDesktopStorageEntry structure which contains required information to register a Storage.
 size - The size of the data structure. It must be set to <code>sizeof(AVDesktopStorageEntryRec)</code>.
 storageKey - ASText, Unique key to identify the storage
 defaultLabel - ASText specifying Label (name) of the connector
 storageAccountAddProc - function pointer to be called when account of this storage is to be added
 storageAccountRemoveProc - function pointer to be called when account of this storage is to be removed
 storageEnableProc - function that tells if storage is Enabled of not
 storageGetIconFromResourceProc - function that returns AVIcon for the given resource ID
 storageGetRootProc - function to get root folder pathname for an account of storage
 otherDataCab - ASCab for future use, in case some more storage data is required
 supportedFeatures - ASInt32, tells which all features are supported by storage like whether more than one account
         of the storage can be configured. Currently only one feature is available i.e. support
         for multiple accounts, other feature support might come in future. Features supported can be clubbed together
		 by doing a bitwise OR of each feature bit. These bits are defined as #defines in AVExpt.h. 
		 Please see: STORAGE_SUPPORT_MULTIPLE_ACCOUNTS
 @param additionalInfo - ASCab for future use, in case some more info is required for the HFT

 This HFT only takes const reference to the struct, it is clients responsibility to free up the memory for struct and its members after usage

 @return status of the storage registration of type AVStorageErrorCode.
 kAVDesktopStorageSuccess if successfully registered the storage
 kAVDesktopStorageBadParams if parameters provided are not proper
 kAVDesktopStorageFileSysNF if filesys specified by fs_name is not registered with Acrobat
 kAVDesktopStorageOtherFailure if other error has occurred during storage registration
 @since PI_ACROVIEW_VERSION >= 0x000C0002
 */
 NPROC(AVStorageErrorCode, AVStorageRegisterNewStorage, (ASAtom fs_name, const AVStorageEntry info, ASConstCab additionalInfo))

 /**
 AVStorageRegisterAccount adds a storage account entry in the Acrobat Storage Accounts
 @param storageKey -  specifies the Desktop Storage for which this account is to be added
 @param accountLabel - Label which will be shown as Name of this account in Left Rail and other places, if NULL label of the desktop storage entry will be used
 @param info - const refernce to ASCab, memory to be managed by client, this cab will be cached by Acrobat and can be retrieved using AVStorageGetAccountInfo,
 can be used to store account specific data. it can contain following entries
 KEY_STORAGE_ACCOUNT_USERID "userId" - value ASText
 KEY_STORAGE_ACCOUNT_USERNAME "userName" - value ASText
 KEY_STORAGE_ACCOUNT_LABEL "label" - value ASText
 KEY_STORAGE_ACCOUNT_TOOLTIP "tooltip" - value ASText
 KEY_STORAGE_ACCOUNT_CLIENTDATA "clientData" - value ASCab for Acrobat this cab is blackbox may be used to cache accessToken or similar
 information, will be encypted before storing to registry/prefs.
 @return ASText specifying account id, unique for the account of this storage, NULL if storageKey is invalid or account could not be added
 @since PI_ACROVIEW_VERSION >= 0x000C0002
 */
 NPROC(ASText, AVStorageRegisterAccount, (ASConstText storageKey, ASConstText accountLabel, ASConstCab info))

 /**
 AVStorageGetAccountInfo returns the storage account information which is cached in Acrobat using AVStorageRegisterAccount
 @param storageKey -  specifies the Desktop Storage
 @param accountId - specifies the account for which information to be retrieved, if NULL/Empty, information of all the accounts of this Desktop Storage are returned
 @return ASCab with the information provided during registerAccount,
 if accountId is not NULL/Empty - ASCab can potentially contain
 KEY_STORAGE_ACCOUNT_USERID "userId" - value ASText
 KEY_STORAGE_ACCOUNT_USERNAME "userName" - value ASText
 KEY_STORAGE_ACCOUNT_LABEL "label" - value ASText
 KEY_STORAGE_ACCOUNT_TOOLTIP "tooltip" - value ASText
 KEY_STORAGE_ACCOUNT_CLIENTDATA "clientData" - value ASCab
 if accountId is NULL/Empty, ASCab will have following
 KEY_STORAGE_ACCOUNT_COUNT "count" - value int specifying number of accounts in this cab
 KEY_STORAGE_ACCOUNT_COLLECTION "accounts" - value ASCab, having 'count' number of cabs each having following two keys :
 KEY_STORAGE_ACCOUNT_ID "id" - value ASText, represents the account id
 KEY_STORAGE_ACCOUNT_INFO "info" - value ASCab having info for particular account id, has following keys :
 KEY_STORAGE_ACCOUNT_USERID "userId" - value ASText
 KEY_STORAGE_ACCOUNT_USERNAME "userName" - value ASText
 KEY_STORAGE_ACCOUNT_LABEL "label" - value ASText
 KEY_STORAGE_ACCOUNT_TOOLTIP "tooltip" - value ASText
 KEY_STORAGE_ACCOUNT_CLIENTDATA "clientData" - value ASCab
 @see AVStorageRegisterAccount
 @since PI_ACROVIEW_VERSION >= 0x000C0002
 */
 NPROC(ASCab, AVStorageGetAccountInfo, (ASConstText storageKey, ASConstText accountId))

 /**
 AVStoragePerformOAuth2Authentication executes the OAuth 2.0 workflow using Code Flow / Authorization Code grant type  and returns access token and other responses from server
 Various keys used here for parameters and in response are taken from OAuth2.0 RFC https://tools.ietf.org/html/rfc6749
 @param standardOauthParamsCab - [required param] ASCab having parameters essential for OAuth 2.0 workflow. It must contain following keys :
 OAUTH2_AUTHORIZEURL "authorizeURL"
 OAUTH2_TOKENURL "accesstokenURL"
 OAUTH2_CLIENTID  "client_id"
 OAUTH2_CLIENTSECRET "client_secret"
 OAUTH2_REDIRECTURI "redirect_uri"
 @param authStepAdditionalData - ASCab having additional parameters for OAuth2.0 Authorization Code step and to configure Browser Control window. It may contain following keys :
 OAUTH2_RESPONSETYPE "response_type" - supported value OAUTH2_RESPONSETYPE_CODEFLOW "code"
 OAUTH2_STATE "state"
 OAUTH2_SCOPE "scope"
 OAUTH2_WINDOW_WIDTH  "window_width"
 OAUTH2_WINDOW_HEIGHT  "window_height"
 OAUTH2_ISWINDOWRESIZABLE "iswindowresizable"
 OAUTH2_SHOWCANCELBUTTONIFIDLE "showCancelButtonIfIdle"
 @param authStepAdditionalData - ASCab having additional parameters for OAuth2.0 Access Token step. It may contain following keys :
 OAUTH2_GRANTTYPE   "grant_type" - supported value OAUTH2_GRANTTYPE_AUTHCODE "authorization_code"
 @param outResultCab - [required param] ASCab in which response will be given. It may contain following keys :
 In case of Success:
 OAUTH2_ACCESSTOKEN "access_token"
 OAUTH2_TOKENTYPE "token_type"
 OAUTH2_REFRESHTOKEN "refresh_token"
 OAUTH2_TOKENEXPIRESIN "expires_in"
 OAUTH2_SCOPE "scope"
 OAUTH2_RESPONSEBODY "responsebody"
 If there is an error response for AccessToken step
 OAUTH2_ERROR "error"
 OAUTH2_ERRORDESCRIPTION "error_description"
 OAUTH2_ERRORURI "error_uri"
 If there is an error response for Authotization code step
 OAUTH2_ERROR "error"
 OAUTH2_ERRORDESCRIPTION "error_description"
 OAUTH2_ERRORURI "error_uri"
 OAUTH2_STATE "state"

 @return OAuth2ErrorCode indicating the status of OAuth call
 kOauth2NoError - successful
 kOauth2AuthorizationUrlNotReachable - no internet while launching authorizeURL
 kOauth2AuthorizationStepError - some error during Authotization code step
 kOauth2AccessTokenUrlNotReachable - - no internet while launching accesstokenURL
 kOauth2AccessTokenStepError -  some error during Access Token step
 kOauth2RequiredParameterMissing - if something from standardOauthParamsCab or outResultCab is missing
 kOauth2BadRequestParameters - if Not supported parameter is provided eg: response_type = token (only code is supported)
 kOAuth2UserClosedWindow - if User has closed the window somewhere during Authorization code step
 kOauth2GeneralError - Some other error

 @since PI_ACROVIEW_VERSION >= 0x000C0002
 */
 NPROC(OAuth2ErrorCode, AVStoragePerformOAuth2Authentication, (ASCab standardOauthParamsCab, ASCab authStepAdditionalData, ASCab accesstokenStepAdditionalHeaderCab, ASCab outResultCab))



 /**
 AVMakeWebServicesRequest method invokes an HTTP Web Service synchronously/asynchronously invokes an HTTP Web Service.
 @param request - AVWebServiceRequest request structure -
 size - The size of the structure. It must be set to <code>sizeof(AVWebRequestParamsRec)</code>.

 verb - String: HTTP verb or NULL to sniff the verb from request mode

 url  - A string specifying the network endpoint location of the web service

 headers - Array of HTTP headers to be sent with the request. Each element in the contains header
 name and values. The Client should copy any header values it required. Header pointers
 and values may be cleaned up by Reader after Response callback scope ends.

 requestBody - A Stream object that specifies the request body of the method.
 No encoding of the message body is done prior to sending it.
 The client should read, copy the stream if required and close it.
 Reader may clean up the stream content after the scope of response callback ends.

 requestWriteProgressProc -  Progress callback used with request Write. Useful for File Upload etc.

 responseReadProgressProc - Progress callback used with responeRead. Useful for File Download etc.

 handler - Callback/AVWebResponseHandlerProc for handling Web Services Response.

 isSync - A boolean that determines whether the Web Service Call is synchronous or asynchronous.

 @since PI_ACROVIEW_VERSION >= 0x000C0002
 */
 NPROC(void, AVMakeWebServicesRequest, (AVWebServiceRequest request))

 /**
 Forces any pending updates for the specified page view to
 finish drawing. And invalited only the updated area instead of complete page
 which was happening in case of AVPageViewDrawNow()
 @param pageView IN/OUT The AVPageView to redraw.
 */
 NPROC(void, AVPageViewDrawNowOptimized, (AVPageView pageView))

#ifdef MAC_PLATFORM
	 /*
	 Checks if Acrobat is the default PDF viewer. If not, then makes it the default viewer.
	 Showing a success dialogue is optional
	 */
	 NPROC(void, AVMakeAcrobatDefault, (bool showSuccesswindow))
#endif // MAC_PLATFORM
...
/**
 ...
 <TR><TH>Operating system</TH><TH>Value</TH></TR>
 <TR><TD>Windows</TD><TD><code>HWND</code>.</TD></TR>
 <TR><TD>Mac OS</TD><TD><code>NSWindow*</code>for Cocoa-based windows (most windows are Cocoa as of 8.0)</TD></TR>
 </TABLE>
 ...
 @see AVWindowNew
 */
NPROC(AVPlatformWindowRef, AVWindowGetPlatformThingEx, (AVWindow win))
