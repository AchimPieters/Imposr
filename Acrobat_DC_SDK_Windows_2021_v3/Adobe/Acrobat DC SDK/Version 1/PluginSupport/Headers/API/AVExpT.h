...
/**
	...
	@see AVWindowGetPlatformThingEx 
...
/** @ingroup cursorID */
#define TEXTMARQUEE_CURSOR			1317
/** @ingroup cursorID */
/** Deprecated. Use TEXTMARQUEE_CURSOR instead. */
#define HIGHLIGHTTEXT_CURSOR		TEXTMARQUEE_CURSOR
/** @ingroup cursorID */
#define STICKYNOTE_CURSOR			1318
/** @ingroup cursorID */
#define HIGHLIGHTIMAGE_CURSOR		1319
/** @ingroup cursorID */
#define PENTOOL_CURSOR				1320
/** @ingroup cursorID */
#define ERASEINK_CURSOR				1321
/** @ingroup cursorID */
#define FSS_TEXT_CURSOR				1400
/** @ingroup cursorID */
#define FSS_CROSS_CURSOR			1401
/** @ingroup cursorID */
#define FSS_CHECKMARK_CURSOR		1402
/** @ingroup cursorID */
#define FSS_CIRCLE_CURSOR			1403
/** @ingroup cursorID */
#define FSS_LINE_CURSOR				1404
/** @ingroup cursorID */
#define FSS_DOT_CURSOR				1405
...
/** The window should have the PDF icon associated with it 
  	@ingroup AVWindowFlags
*/
#define AVWIN_PDFICON			0x00080000

/** 
 * The window ignores all mouse events.
 * @ingroup AVWindowFlags
 */
#define AVWIN_IGNORESMOUSE		0x00040000

/**
* The window does not active on mouse event, but does not discard mouse messages.
* @ingroup AVWindowFlags
*/
#define AVWIN_NOACTIVATE		0x00020000

/**
* The window enables the drop shadow effect.
* @ingroup AVWindowFlags
*/
#define AVWIN_DROPSHADOW		0x00010000

/** The window honors app theme.
	@ingroup AVWindowFlags
*/
#define AVWIN_THEMED			0x00008000

/**
* The window merges menu bar into the title bar.
* @ingroup AVWindowFlags
*/
#define AVWIN_MERGEMENUBAR		0x00004000
...
#if MAC_PLATFORM
    ,
    /** Show preference in dialog for using service for conversion of selected file **/
    kAVOpenShowConversionServiceUsagePreference   = 1 << 8
#endif
};
...
typedef struct {								
	...
	*/
	...
	/** Acrobat Only. The name of the first file option i.e. Reduce File size or Compress PDF that is used for the prompt. */
	ASText										fileOptionsRFS;

	// Custom input data to be passed to open/save APIs
	ASSize_t									customInputDataSize;
	void*										customInputData;
	// Custom output data from open/save APIs
	ASSize_t									customOutputDataSize;
	void* customOutputData;
...
typedef enum {
	...
	*/
	...
	/** User and application defined App files in reader mode of unified app */
	kAVSFRdrApp,

	/** User and application defined UI theme files */
	kAVSFUIThemes,

	/** default folder to store accessibility reports*/
	kAVSFAccReports,

	/** default folder to store Onix32.dll in 64 Bit Acrobat*/
	kAVSFONIXLIB,

	/** Temporary folder for Onix Process.
	*/
	kAVSFOLTemp,
...
enum {
	...
	/** The conversion failed as DC APIs are not supported for the version of Acrobat */
	kAVConversionFailureDCAPIUnsupported,
    
#if MAC_PLATFORM
    /* The conversion failed because Acrobat app donot have permission to access Office apps. This is required for MacOS 10.14 onwards*/
    kAVConversionFailureOfficeAccessDenied,
#endif //MAC_PLATFORM

	/** The conversion failed as it is not trusted */
	kAVConversionNotTrusted
    
#if MAC_PLATFORM
    // Post Catalina due to file privacy features target converter application like Distiller
    // may not have access to file, hence conversion failed.
    , kAVConversionDistillerAccessDenied
#endif //MAC_PLATFORM
};
...
**********************************************************************************/
    ...
    /*
     *  Various error codes used for remote storage and Acrobat interaction
     */
    enum {
        kAVStorageSuccess = 0,
        kAVStorageBadParams,
        kAVStorageFileSysNF,
        kAVStorageAccessTokenExpired,
        kAVStorageOtherFailure,
    };
    
    typedef ASEnum8 AVStorageErrorCode;

	/*
	*  Enum for UI theme Icon Set
	*/
	enum {
		kAVStorageLightThemeIcon = 0,
		kAVStorageDarkThemeIcon
	};

	typedef ASEnum8 AVStorageIconSet;
    
    /**
     Callback Proc to add a new account of the remote storage.
     
     @param storageKey - The key of storage for which Add Account workflow is to be performed.
     @param accountId - To be filled by the storage plugin. The caller owns this ASText, so the plugin should not destroy it.
     @param otherData - reserved for future use. Should not be used by the storage plugin. The storage plugin should not destroy it.
     @return ASBool - TRUE if account is successfully added. FALSE otherwise.
     */
    typedef ASBool(*AVStorageAddAccountProc)(ASConstText storageKey, ASText accountId, ASCab otherData);
    
    /**
     Callback Proc to remove an existing account of the remote storage.
     
     @param storageKey - The key of storage for which Remove Account workflow is to be performed.
     @param accountId - The account id of the account to be removed.
     @return ASBool - TRUE if account is successfully removed. FALSE otherwise.
     */
    typedef ASBool(*AVStorageRemoveAccountProc)(ASConstText storageKey, ASConstText accountId);
    
    /**
     Callback Proc to determine if this remote storage is enabled.
     
     @param storageKey - The key of the storage.
     @return ASBool - TRUE if this storage is enabled. FALSE otherwise. If this storage is not enabled, then it will not appear anywhere in Acrobat.
     */
    typedef ASBool(*AVStorageIsEnabledProc)(ASConstText storageKey);
    
	/**
	Callback Proc to get the AVIcon for the resource IDs mentioned by remote storage

	@param storageKey - The key of storage for which icon is required.
	@param iconSet - The icon set tells whether the icon required is for Dark Theme or not
	@param width - Specifies the Width of the icon
	@param height - Specifies the Height of the icon
	@param isPDF - The flag which tells whether the icon is extracted from pdf or png
	The plugin needs to generate an AVIcon and return it.
	@return AVIcon - The icon for the resource id.

	There are 6 type of icons which the proc would be providing based on the above parameter
	For each Dark UI theme and Normal theme below three icons would be provided

	ICON SIZE                            ICON_TYPE                        ICON_USAGE
	---------							-----------						--------------
	320px X 320px							png							 Add Account View
	40 X 40 px								pdf							 Recent File list in HomeView and FileMenu
	40 X 40 px								png							 Recent File list in Custom Open Dialog

	*/
	typedef AVIcon(*AVStorageGetIconFromResourceIDProc)(ASConstText storageKey, AVStorageIconSet iconSet, ASInt32 width, ASInt32 height, ASBool isPDF);
    
    
    /**
     Callback Proc to get the root folder ASPathname for an existing storage account
     
     @param storageKey - The key of storage whose root pathname method is to be invoked.
     @param accountId - The account id of the account for which root pathname is required.
     @param retPath - A pointer to an ASPathName. The plugin needs to create the ASPathName and fill in this parameter.
     @return AVStorageErrorCode - one of the error codes. Should return kAVStorageSuccess if pathName is successfully generated.
     */
    typedef AVStorageErrorCode(*AVStorageGetRootProc)(ASConstText storageKey, ASConstText accountId, ASPathName* retPath);
    
    /**
     * Data structure that contains the information required by Acrobat to register a Storage.
     */
    typedef struct
    {
        /** The size of the data structure. It must be set to <code>sizeof(AVStorageEntryRec)</code>. */
        ASSize_t size;
        
        /** Unique key to identify the storage. */
        ASText                      storageKey;
        
        /** Name of the storage visible in UI */
        ASText                      defaultLabel;
        
        /** Callback Proc to add a new account of the remote storage. */
        AVStorageAddAccountProc     storageAccountAddProc;
        
        /** Callback Proc to remove an existing account of the remote storage */
        AVStorageRemoveAccountProc  storageAccountRemoveProc;
        
        /** Callback Proc to determine if this remote storage is enabled */
        AVStorageIsEnabledProc      storageEnableProc;
		...
		*/
        ...
        AVStorageGetIconFromResourceIDProc storageGetIconFromResourceProc;
        
        /** Callback Proc to get the root folder ASPathname for an existing storage account. */
        AVStorageGetRootProc		storageGetRootProc;
        
        /** ASCab for future use. It will be set to null and the client should not use it.  */
        ASCab                       otherDataCab;
        
		/** Tells which all features are supported by storage like whether more than one account
		of the storage can be configured. Currently only one feature is available i.e. support
		for multiple accounts, other feature support might come in future. Features supported
		can be set by doing bitwise OR using #define for Hex codes for supported features. */
        ASInt32                     supportedFeatures;
    } AVStorageEntryRec, *AVStorageEntry;
    
    /**
     Structure for HTTP Headers
     ...
     Rfc2616-sec14 contains detailed information about each supported header.
     A list of headers supported today is present here -
     http://www.iana.org/assignments/message-headers/message-headers.xml#perm-headers
     */
    typedef struct _t_AVHTTPHeaderRec {
        /** HTTP header Name. */
        const char* name;
        
        /** HTTP Header Value */
        const char* value;
    } AVHTTPHeaderRec, *AVHTTPHeader;
    
    /**
     Array of HTTP Headers
     */
    typedef struct _t_AVHTTPHeaderArrayRec {
        /** Number of Header fields in the array. */
        ASUns32		count;
        
        /** Array of headers. */
        AVHTTPHeaderRec* headers;
    } AVHTTPHeaderArrayRec, *AVHTTPHeaderArray;

     /**
     Array of HTTP Headers Names that are used to prepopulate them from response 
     */
    typedef struct _t_AVHTTPHeaderNamesArrayRec {
		/** Number of Header fields in the array. */
		 ASUns32     count;

 		/** Array of headers Name */
		 const char** headerNamesArray;
	} AVHTTPHeaderNamesArrayRec, *AVHTTPHeaderNamesArray;

    
    /**
     Structure for HTTP Web Services Response.
     @See AVWebRequestHandlerProc
     */
    typedef struct _t_AVWebServiceResponseRec {
        /** The size of the structure. Set it to <code>sizeof(AVWebServiceResponseRec)</code>. */
        ASSize_t		size;
        
        /**  HTTP status code returned by Web Service, or -1 on Network errors.*/
        ASInt32 statusCode;
        
        /** Array of HTTP headers to be sent with the request. Each element in the contains header
         name and values.
         The Client should copy any header values it required. Header pointers
         and values may be cleaned up by Reader after Response callback scope ends.
         */
        AVHTTPHeaderArray				headers;
        
        /** The response body as a stream object on success. NULL on failure.
         The client should read, copy the stream if required, and close it.
         Application may clean up the stream content after the scope of callback ends.
         */
        ASStm	responseBody;
        ...
        /**  An error message associated with the failure if any.*/
        ASText errorMessage;
    } AVWebServiceResponseRec, *AVWebServiceResponse;
    
    /**
     A callback for handling Web Services Response. It returns the response for a Web Service request made
     with AVMakeWebServicesRequest.
     
     @param response - AVWebServiceResponse.
     */
    typedef ACCBPROTO1 void (ACCBPROTO2 *AVWebResponseHandlerProc)(AVWebServiceResponse response);
    
    
    /* Progress callback used with AVMakeWebServicesRequest
     *
     * @param bytesWritten	Count of the bytes written so far.
     * @param bytesTotal	Total size of the request body.
     * @param clientData	User-supplied data.
     */
    typedef ACCBPROTO1 void (ACCBPROTO2 *AVWebRequestProgressProc)(
                                                                   ASByteCount bytesWritten,
                                                                   ASByteCount bytesTotal,
                                                                   void *clientData);
    
    
    /**
     Structure for HTTP Web Services Request.
     @see AVMakeWebServicesRequest.
     The caller retains the ownership and responsibility to free AVWebServiceRequest, headers cab and/or request stream.
     */
    typedef struct _t_AVWebServiceRequestRec
    {
        /** The size of the structure. It must be set to <code>sizeof(AVWebRequestParamsRec)</code>. */
        ASSize_t 			size;
        
        /**
         The HTTP verb or NULL to sniff the verb from the open mode given to
         ASFileSysOpenFile.
         
         See RFC 2616 " Hypertext Transfer Protocol -- HTTP/1.1" Section 9
         for a description of useful values for the "Verb" field.
         
         This string will not be copied and may be referenced until is unloaded.
         */
        const char*			verb;
        ...
        /** A Stream object that specifies the request body of the method. No encoding of the message body is done prior to sending it.
         Client should not close the stream, the stream will be closed by reader after making the call
         */
        ASStm				requestBody;
        
        /** Callback for handling Web Services Response. */
        AVWebResponseHandlerProc handler;
        
        /* Progress callback used with request Write. Useful for File Upload for eg. */
        AVWebRequestProgressProc requestWriteProgressProc;
        
        /* Progress callback used with responeRead. Useful for File Download etc. */
        AVWebRequestProgressProc responseReadProgressProc;
        
        /** A pointer to user-defined data passed to the callback (handler). */
        void*	clientData;
        
        /** A pointer to user-defined data passed to the progress callback (requestWriteProgressProc/ responseReadProgressProc). */
        void*	progressCBClientData;
        
        /** A boolean that determines whether the Web Service Call is synchronous or asynchronous. */
        ASBool				isSync;

		/** Array of header names required in the HTTP response.
		The header values would be populated in AVWebServiceResponseRec struct by parsing response headers of the web request.**/
		AVHTTPHeaderNamesArray responseHeaderNamesArray;

    } AVWebServiceRequestRec, *AVWebServiceRequest;
    
    
#define EUREKA_REVIEW_DOC_OPENED_NOTIFICATION "EurekaReviewDocOpened"


#define EUREKA_REVIEW_FETCH_REVIEW_COMMAND "EurekaReviewFetchReview"
#define EUREKA_REVIEW_CLOSE_REVIEW_COMMAND "EurekaReviewCloseReview"
#define EUREKA_REVIEW_ADD_PARTICIPANT_COMMAND "EurekaReviewAddParticipant"
#define EUREKA_REVIEW_REMOVE_PARTICIPANT_COMMAND "EurekaReviewRemoveParticipant"
#define EUREKA_REVIEW_GET_PARTICIPANT_COMMAND "EurekaReviewGetParticipant"
    
#define EUREKA_REVIEW_UPDATE_PARTICIPANT_COMMAND "EurekaReviewUpdateParticipantCommand"
#define EUREKA_REVIEW_MODEL_UPDATE_COMMAND "EurekaReviewModelUpdateCommand"

   

	typedef struct{
		ASText eurekaCommand;
		ASText reviewId;
		ASText invitationId;
		ASText etagParticipant;
		ASText participantId;
        ASText parcelId;
        
	}EurekaReviewInfoRec, *EurekaReviewInfo;


	typedef ACCBPROTO1 void (ACCBPROTO2 *EurekaPerRequestCompletionProcAcrobat)(ASInt32 errorCode, void* data, void* clientData, EurekaReviewInfo reviewInfo);
    
    
    typedef struct{
		EurekaReviewInfoRec eurekaReviewInfo;
        EurekaPerRequestCompletionProcAcrobat callBackProc;
        void* clientData;
		void* data;
        //ASBool useCached;        //Removed as for now we do not maintain cache
	}EurekaRequestInfoRec, *EurekaRequestInfo;
    
    
    
    
	/**
	* Hex codes used for setting and checking which all features that are supported in AVStorageEntryRec. It can be set by doing
	the bitwise OR of the following #define corresponding to all the features that are supported.
	*/
#define STORAGE_NO_FEATURE_SUPPORTED 0x00000000
#define STORAGE_SUPPORT_MULTIPLE_ACCOUNTS 0x00000001
	
	/**
     Strings used for Cab keys in Information Cab passed to AVStorageRegisterAccount or retrived from AVStorageGetAccountInfo APIs
     */
#define KEY_STORAGE_ACCOUNT_USERID			"userId"
#define KEY_STORAGE_ACCOUNT_USERNAME		"userName"
#define KEY_STORAGE_ACCOUNT_LABEL			"label"
#define KEY_STORAGE_ACCOUNT_TOOLTIP			"tooltip"
#define KEY_STORAGE_ACCOUNT_DATA            "accountData"
#define KEY_STORAGE_ACCOUNT_CLIENTDATA		"clientData"
#define KEY_STORAGE_ACCOUNT_COUNT			"count"
#define KEY_STORAGE_ACCOUNT_COLLECTION		"accounts"
#define KEY_STORAGE_ACCOUNT_ID				"id"
#define KEY_STORAGE_ACCOUNT_INFO			"info"
    
    
    /**
     Strings used for Cab keys in AVStoragePerformOAuth2Authentication to execute the OAuth 2.0 workflow using Code Flow / Authorization Code grant type
     Various keys used here for parameters and in response are taken from OAuth2.0 RFC https://tools.ietf.org/html/rfc6749
     
     These keys are parameters essential for OAuth 2.0 workflow.
     */
    
#define OAUTH2_AUTHORIZEURL					"authorizeURL"
#define OAUTH2_TOKENURL						"accesstokenURL"
#define OAUTH2_CLIENTID						"client_id"
#define OAUTH2_CLIENTSECRET					"client_secret"
#define OAUTH2_REDIRECTURI					"redirect_uri"
    
    /**
     Keys for additional Paramter cabs for AVStoragePerformOAuth2Authentication. They are used to configure Browser Control window and may contain following keys:
     */
#define OAUTH2_RESPONSETYPE					"response_type"
#define OAUTH2_RESPONSETYPE_CODEFLOW		"code"
#define OAUTH2_RESPONSETYPE_IMPLICITGRANT	"token"
#define OAUTH2_STATE						"state"
#define OAUTH2_SCOPE						"scope"
    
#define OAUTH2_WINDOW_WIDTH					"window_width"
#define OAUTH2_WINDOW_HEIGHT				"window_height"
#define OAUTH2_ISWINDOWRESIZABLE            "iswindowresizable"
#define OAUTH2_SHOWCANCELBUTTONIFIDLE       "showCancelButtonIfIdle"
    
#define OAUTH2_CODE							"code"
    
#define OAUTH2_GRANTTYPE					"grant_type"
#define OAUTH2_GRANTTYPE_AUTHCODE			"authorization_code"
    
    /**
     Keys for ASCab in which response will be given. It may contain following keys :
     */
#define OAUTH2_ACCESSTOKEN					"access_token"
#define OAUTH2_TOKENTYPE					"token_type"
#define OAUTH2_REFRESHTOKEN					"refresh_token"
#define OAUTH2_TOKENEXPIRESIN				"expires_in"
    
    //these keys will be filled in the outcab provided by the user
#define OAUTH2_RESPONSECODE					"responsecode"
#define OAUTH2_OAUTHERRORCODE				"errorcode"
#define OAUTH2_RESPONSEBODY					"responsebody"
    
    //Other Headers for Error Response
#define OAUTH2_ERROR						"error"
#define OAUTH2_ERRORDESCRIPTION				"error_description"
#define OAUTH2_ERRORURI						"error_uri"
    
    /**
     OAuth2ErrorCode indicating the result of OAuth call made with AVStoragePerformOAuth2Authentication API
     */
    typedef enum
    {
        kOauth2NoError,
        kOauth2AuthorizationUrlNotReachable,
        kOauth2AuthorizationStepError,
        kOauth2AccessTokenUrlNotReachable,
        kOauth2AccessTokenStepError,
        kOauth2RequiredParameterMissing,
        kOauth2BadRequestParameters,
        kOAuth2UserClosedWindow,
        kOAuth2NetworkFailure,
        kOauth2GeneralError
    } OAuth2ErrorCode;
